# Contributing to Milpa Live Web

Thanks for your interest in contributing! Milpa Live Web is the web surface for
`milpa/live` — HTML renderers over the design system, XHTML state/interaction
codecs, and a security-hardened `LiveEndpoint` (HMAC-signed state, CSRF,
single-use nonce/replay protection). It builds on `milpa/live` for the
render-target-agnostic component lifecycle and `milpa/core` for attributes,
interfaces, and events, with no product coupling.
...
- **Respect the tier boundary.** Milpa Live Web depends on `milpa/live` and
  `milpa/core`, never the reverse. Do not introduce a dependency on Doctrine, a
  concrete container, or any product/plugin code, and do not push live-web
  concerns down into `milpa/live` or the core.
- **Treat the security package as load-bearing.** `src/Security` implements the
  HMAC state-signing, CSRF, and replay-nonce guarantees `LiveEndpoint` relies on
  to trust client-echoed state. Changes there need tests that pin the tamper- and
  replay-rejection behavior, not just the happy path.
...
The whole Milpa family (`milpa/core`, `milpa/http`, `milpa/live`, `milpa/live-web`,
